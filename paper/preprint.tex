...
\paragraph{Objective.} MDLM \citep{sahoo2024mdlm}: per sequence draw $t\sim U(0.01,0.99)$, mask each token independently with probability $t$ (model vocab $16{,}001$: the $16{,}000$ SentencePiece pieces plus the corruption symbol \MASK\ at id 16000), cross-entropy on masked positions only. SFT is DiffuSeq-style: only response positions are noised, prompts stay clean as conditioning; zero new parameters, pretrain checkpoint loaded \texttt{strict=True}.

\paragraph{Tokenizer.} A 16{,}000-piece SentencePiece BPE was trained from scratch on the English pretrain corpus --- byte-fallback (no out-of-vocabulary tokens), reserved \texttt{<pad>/<unk>/<s>/</s>} at ids 0--3 --- rather than reusing an off-the-shelf vocabulary, so subword units track the clinical register (\emph{psychotherapy}, \emph{depression}, \emph{cognitive}, \emph{schizophrenia} each map to a single piece). 16k was chosen as a compact vocabulary for this parameter budget --- the embedding table is material at \sixsix\ --- and vocabulary size was not ablated. The \MASK\ corruption symbol is one extra embedding row appended at id 16000 (model vocab 16001), not an SPM piece. The same frozen tokenizer is used by every arm including the continuous baseline (\S\ref{sec:cont}), so it confounds no reported contrast, and all token counts and the tok/param figures (Table~\ref{tab:arch}) are in its units; the exact tokenizer ships as \repo{spm\_en.model} with the model release.
