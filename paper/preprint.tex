...
(2)~\textbf{Blind} (decisive): 56 pooled, shuffled, source-stripped, two fresh annotators:
...
\textbf{Aggregate paired:} 0.490 vs 0.500, $d=-0.0094$, CI $[-0.0188,-0.0001]$, $t=-1.96$, win 0.479 --- practical tie. \textbf{Blind traps} (3 fresh annotators, $\kappa$ 0.767/0.867/0.825): \onetwenty\ 7/28 vs \sixsix\ 9/28, Newcombe $[-0.294,+0.161]$, Fisher $p=0.768$ --- floor 25--32\% both. \textbf{Led to:} measured null; inferred data-bound mechanism (6.4 vs 11.6 tok/param, optimum ${\approx}20$); production $=$ \sixsix-clean. Scope: single seed/cell, two checkpoints, in-distribution golds; falsifier $=$ 2.4B-token \onetwenty.

% =====================================================================
\section{Stage 7 --- on-device deployment; conclusions}\label{sec:deploy}
% =====================================================================

\sixsix-clean exported to ONNX (286~MB asset; debug APK 365~MB), ONNX Runtime Mobile CPU, fp32 inference of bf16 weights (no quantization), v2 sampler ported, diffusion visualized (Figure~\ref{fig:app}). \textbf{Conclusions:} discrete $>$ continuous at small scale; collapse is a decode problem; cleaning buys cost; doubling params at fixed data buys nothing measured; \emph{blind annotation is non-negotiable}.
...
\caption{The \sixsix-clean model on a phone (ONNX Runtime Mobile, 8 CPU threads). Left: masked-diffusion decoding mid-generation (step 4/18) --- unmasked tokens appear in confidence order. Right: completed chat answers with the \S\ref{sec:decode} sampler (steps 28, temp 0.9, rep 1.3). Outputs are grammatical, on-topic, mildly repetitive --- consistent with the limitations stated for this scale.}
\label{fig:app}
\end{figure}
...
ONNX asset ``$\approx$243 MB'' & app assets dir & 286.4 MB; \fixed \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{1000-question curators (\S\ref{sec:data}--\ref{sec:scale}).} Candidate questions were machine-extracted in-distribution from the three SFT sources, psych-filtered, deduplicated, and split into 20 shards of 150; each of 20 sub-agents was instructed to curate, from its shard, the best 50 distinct well-formed English psychology questions, with a further 8 agents reconciling overlaps to exactly 1000; gold answers were recovered for 725 by matching SFT targets. Scripts: \repo{build\_q\_shards.py}, \repo{recover\_answers.py}. Curators never saw model outputs, so the prompt set predates and is independent of every model under comparison.
...
\bibitem[Sahoo et al.(2024)]{sahoo2024mdlm} Sahoo et al. Simple and Effective Masked Diffusion Language Models. NeurIPS 2024.
\bibitem[Lou et al.(2024)]{lou2024sedd} Lou et al. Discrete Diffusion Modeling via Score Entropy. ICML 2024.
\bibitem[Chang et al.(2022)]{chang2022maskgit} Chang et al. MaskGIT: Masked Generative Image Transformer. CVPR 2022.
\bibitem[Li et al.(2022)]{li2022diffusionlm} Li et al. Diffusion-LM Improves Controllable Text Generation. NeurIPS 2022.
\bibitem[Gong et al.(2023)]{gong2023diffuseq} Gong et al. DiffuSeq. ICLR 2023.
\bibitem[Hoffmann et al.(2022)]{hoffmann2022chinchilla} Hoffmann et al. Training Compute-Optimal Large Language Models. 2022.
\bibitem[Eldan and Li(2023)]{eldan2023tinystories} Eldan, Li. TinyStories. arXiv:2305.07759.
\bibitem[Gunasekar et al.(2023)]{gunasekar2023textbooks} Gunasekar et al. Textbooks Are All You Need. arXiv:2306.11644.
\bibitem[Peebles and Xie(2023)]{peebles2023dit} Peebles, Xie. Scalable Diffusion Models with Transformers. ICCV 2023.
\bibitem[Dettmers et al.(2022)]{dettmers2022adamw8bit} Dettmers et al. 8-bit Optimizers via Block-wise Quantization. ICLR 2022.
\end{thebibliography}
\end{document}
